...
## Honrar a Premissa
Deparados com a proposta **"jogo de adivinhar números de 0 a 100"**, estabelecida para o projeto integrador nesse período, quebramos bastante a cabeça pensando quais caminhos poderíamos seguir em um limitador tão claro e preciso. **Como fazer algo interessante a partir disso?** Cogitamos desenvolver algo mais "fora-da-caixa" que reinterpretasse, expandisse ou até distorcesse um pouco a proposta inicial — e supomos que outras equipes devem ter passado pelo mesmo dilema.
 
Eventualmente chegamos em um direcionamento para o projeto, mudando um pouco a nossa pergunta inicial: de *"Como fazer algo interessante a partir disso?"* para **"Como fazer isso ser interessante?"**. Decidimos honrar 100% a premissa e abandonar qualquer ideia que tirasse o foco dela. Tomamos como nosso desafio utilizar os princípios de IHC, Programação, Engenharia de Software e Gestão de Projetos que aprendemos ao longo do período para tornar um jogo com uma premissa tão simples em algo interessante e divertido.


---

## Interface Numérica
Para trazer dinamismo sem abandonar a premissa, focamos inicialmente em pensar em um modo de interação principal que fosse mais interessante e intuitivo do que simplesmente usar o teclado para inputar palpites. Chegamos na ideia da linha numérica (ou "régua") e sua interação com o mouse. Esse componente serve o design de Numeral em três frentes ao mesmo tempo:
...
A partir dessa interação central, desenvolvemos toda a interface.

A partir dessa interação central, desenvolvemos o restante da interface em um protótipo animado, simulando uma partida do jogo:

[![Thumbnail do video de demonstracao](./assets/github/thumb_video.png)](https://www.youtube.com/watch?v=L74IL4UUWyY)
[*Simulação UI*](https://www.youtube.com/watch?v=L74IL4UUWyY)
...
[*Protótipo Figma*](https://www.figma.com/proto/UmOYtPDtwu6C4vAOKjSVRf/Pablo-Software-s-Numbers?node-id=107-60&starting-point-node-id=107%3A60&t=1Hx9YyfoWuX6dcAD-1)
...
O jogo pode ser facilmente compilado para Linux (Debian, Ubuntu), macOS (Silicon) e Windows. Utilizamos a linguagem de programação **C** (standard C11) e a biblioteca **[Raylib](https://raylib.com)** para o desenvolvimento.
 
Na camada de lógica, a maioria das funções manipula via ponteiros a estrutura `Session *game`, que armazena os dados atuais e o histórico da partida. Na camada de interface, consumimos esses dados para atualizar a visualização do jogo. Funções recursivas foram utilizadas para o cálculo de soma e desvio padrão (`somaRec()` e `desvioRec()`), na tela de análise de estatísticas.
...
Para isso, preparamos algumas intruções que podem ser localizadas em **[CONTRIBUTING.md](docs/CONTRIBUTING.md)**.
...
| Nome | Área |
|------|------|
| [Lucas Bonfim](https://github.com/l-bonfim) | Interface · Lógica (estatísticas) |
